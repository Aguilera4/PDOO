...
module InterfazTextualQytetet
  ...
  def elegir_quiero_comprar
    mostrar( '¿Desea comprar esta propiedad?')   
    menuSC=[[0, 'NO'], [1, 'SI']]
    salida=seleccion_menu(menuSC)
    mostrar( 'has elegido')
    mostrar(salida)
...
def menu_elegir_propiedad(listaPropiedades) # numero y nombre de propiedades            
    ...
    puts menuEP.inspect
    salida=seleccion_menu(menuEP); # MÈtodo para controlar la elecciÛn correcta en el men˙ 
    salida;
end  
   ...
   def obtener_nombre_jugadores
       ...
       begin
        ...
        valido=comprobar_opcion(lectura, 2, 4); #mÈtodo para comprobar la elecciÛn correcta
      end while(!valido)
